...
task configVMNetworkIP {
    ...
    foreach ($Clone in $CloneArray) {
        ...
        if (!run) { 
            throw "$($Clone.Name): Network IP change to $($Config.virtualMachines[$i].testIp) failed. GuestOS $($Config.virtualMachines[$i].guestOS) is not supported (Windows/Linux). Cloned VMs remain cloned status, manual cleanup might needed!" 
        }
